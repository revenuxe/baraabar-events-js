  ...
  return (
    <header className={`${home ? "sticky md:fixed" : "sticky"} inset-x-0 top-0 z-40 border-b border-[#e8edf3] bg-white shadow-sm`}>
      <div className="mx-auto flex h-[72px] max-w-7xl items-center gap-4 px-4 md:px-8">
        <Link href="/" className="flex shrink-0 items-center"><Image src={logo} alt="Decor Eventz" priority className="h-12 w-44 object-contain md:h-14 md:w-64" /></Link>
        ...
        <div className="ml-auto flex items-center gap-2 md:gap-5">
          <button onClick={() => setSearchOpen(true)} className="grid h-10 w-10 place-items-center rounded-full text-primary md:hidden"><Search className="h-5 w-5" /></button>
      ...
      <nav className="no-scrollbar relative flex h-11 items-center gap-5 overflow-x-auto border-t border-[#edf0f4] px-4 pr-14 md:hidden">
        {NAV_ITEMS.map((item) => <button key={item.slug} onClick={() => setMobileMenu(item.slug)} className="inline-flex shrink-0 items-center gap-1 text-sm font-semibold text-primary">{item.label}<ChevronDown className="h-3.5 w-3.5 text-muted-foreground" /></button>)}
      ...
      {mobileMenu && <div className="fixed inset-x-0 bottom-0 top-[116px] z-50 overflow-y-auto bg-white p-5 md:hidden"><div className="flex items-start justify-between"><div><p className="text-xs font-bold uppercase tracking-[.18em] text-accent">Explore</p><h2 className="mt-1 text-2xl font-bold text-primary">{mobileItem?.label}</h2></div><button onClick={() => setMobileMenu(null)} aria-label="Close menu" className="grid h-10 w-10 place-items-center rounded-full border border-[#dce4ed] text-primary"><X className="h-5 w-5" /></button></div><p className="mt-6 text-xs font-bold uppercase tracking-[.16em] text-muted-foreground">Subcategories</p><div className="mt-3 grid grid-cols-2 gap-3">{mobileSubcategories.length ? mobileSubcategories.map((subcategory) => <Link key={subcategory.slug} href={`/categories/${mobileMenu}/sub/${subcategory.slug}`} onClick={() => setMobileMenu(null)} className="rounded-xl border border-[#dce4ed] px-4 py-4 text-sm font-semibold text-primary">{subcategory.name}</Link>) : <Link href={`/categories/${mobileMenu}`} onClick={() => setMobileMenu(null)} className="rounded-xl border border-[#dce4ed] px-4 py-4 text-sm font-semibold text-primary">View all {mobileItem?.label}</Link>}</div>{mobileProducts.length > 0 && <><p className="mt-7 text-xs font-bold uppercase tracking-[.16em] text-muted-foreground">Popular setups</p><div className="mt-3 grid grid-cols-2 gap-3">{mobileProducts.map((product) => <Link key={product.slug} href={`/categories/${mobileMenu}/${product.slug}`} onClick={() => setMobileMenu(null)} className="overflow-hidden rounded-xl border border-[#dce4ed]"><div className="relative aspect-square">{product.image && <Image src={product.image} alt={product.name} fill sizes="45vw" className="object-cover" />}</div><p className="line-clamp-2 p-3 text-sm font-semibold text-primary">{product.name}</p></Link>)}</div></>}</div>}
      <nav className="hidden border-t border-[#edf0f4] md:block"><div className="mx-auto flex max-w-7xl items-center gap-7 px-8 py-3">{NAV_ITEMS.map((item) => {
